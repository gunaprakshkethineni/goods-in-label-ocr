...
# what identifies a physical part. the barcode if we got it, otherwise the serial off the text
def key_for(row):
    return row.get("barcode") or row.get("serial") or ""
...
def log_scan(row, flags, source):
    ...
    # a serial that has already been through this session is a second physical part carrying a
    # number that is meant to be unique. either somebody scanned it twice or the line printed a
    # duplicate, and both of those want a person
    if key and any(s["key"] == key for s in scans):
        flags = flags + ["DUPLICATE_SERIAL"]
...
def summary():
    ...
    return {"total": total,
            ...
            "rows": [{k: s[k] for k in
                      ["n", "when", "source", "part_no", "serial", "lot_no", "qty", "flags"]}
...
def scan():
    ...
    fixed, flags = check_row(dict(row, filename=source), approved)
...
def export():
    ...
    cols = ["n", "when", "source", "part_no", "serial", "lot_no", "qty", "barcode", "status", "flags"]
    with open(EXPORT_FILE, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=cols)
        w.writeheader()
        for s in scans:
            w.writerow({"n": s["n"], "when": s["when"], "source": s["source"],
                        "part_no": s["part_no"], "serial": s["serial"], "lot_no": s["lot_no"],
                        "qty": s["qty"], "barcode": s["barcode"],
                        "status": "CHECK" if s["flags"] else "OK",
                        "flags": ";".join(s["flags"])})
...
<header>
  <h1>Goods-in label scanner</h1>
  <p>Read a label, check it, keep the running list. Same reader and same checks as the batch tool.</p>
</header>
...
<div class="wrap">
  <div class="col">
    <div class="box" id="pane-up">
      <h2>Upload</h2>
      <div id="drop">Drop label images here, or click to pick them</div>
      <input type="file" id="file" accept="image/*" multiple hidden>
      <div class="hint">You can pick several at once. <code>data/labels/</code> is full of them.</div>
    </div>
...
<div class="logwrap">
  <div class="box">
    ...
    <div class="stats">
      <div class="stat"><b id="s_total">0</b><span>scanned</span></div>
      <div class="stat"><b id="s_pass">0</b><span>into inventory</span></div>
    ...
    <div class="row" style="margin:0 0 14px">
      <button id="imp" class="grey">Add the 60 batch labels</button>
...
function drawLog(s) {
  ...
  let h = "<table><tr><th>#</th><th>time</th><th>source</th><th>part</th><th>serial</th>" +
          "<th>lot</th><th>qty</th><th>status</th></tr>";
  for (const r of s.rows) {
    const ok = r.flags.length === 0;
    h += "<tr><td>" + r.n + "</td><td>" + r.when + "</td><td>" + r.source + "</td><td>" +
         (r.part_no || "-") + "</td><td>" + (r.serial || "-") + "</td><td>" +
         (r.lot_no || "-") + "</td><td>" + (r.qty || "-") + "</td><td>" +
...
function show(r) {
  ...
  v.textContent = r.ok ? "PASS - added to inventory" : "NEEDS CHECK - held for a person";
  ...
  if (!r.logged) m += " &middot; same label as the last one, not logged again";
